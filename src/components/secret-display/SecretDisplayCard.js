...
const GridItem = styled(Row).attrs({
  ...
  height: fonts.lineHeight.bigger,
});
...
function SeedWordGrid({ seed }) {
  const columns = useMemo(() => {
    ...
    return [words.slice(0, words.length / 3), words.slice(4, 8), words.slice(8, 12)];
  }, [seed]);
  ...
  return (
    <RowWithMargins margin={24}>
      {columns.map((wordColumn, colIndex) => (
        <RowWithMargins key={wordColumn.join('')} margin={0}>
            ...
            {times(wordColumn.length, index => {
              ...
              return (
                <GridItem 
                  ...
                  paddingLeft={10}
                  backgroundColor={colors.yellowTrasparent}>
                  <GridText
                    align="right"
